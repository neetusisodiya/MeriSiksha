...
import com.muravtech.merishiksha.server.Config;
...
public class AdminAdapter extends RecyclerView.Adapter<AdminAdapter.VideoViewHolder> {
    Context context;
    ArrayList<AdminForChat.DataBean> listSubjectName;
    OnItemClickListener onItemClickListener;


    public AdminAdapter(Context context, ArrayList<AdminForChat.DataBean> listSubjectName, OnItemClickListener onItemClickListener) {
    ...
    public void onBindViewHolder(@NonNull VideoViewHolder holder, int position) {
        ...
        holder.chatroomUserName.setText(listSubjectName.get(position).getSchool_name());
        if(listSubjectName.get(position).getImage()!=null && !listSubjectName.get(position).getImage().equalsIgnoreCase("")){
            Picasso.with(context).load(Config.BASE_URL_IMAGE+listSubjectName.get(position).getImage()).placeholder(R.drawable.defaultpic).into(holder.chatroomUserImg);
        }
