...
public class ExeleFileActivity extends AppCompatActivity implements OnClickListener {
    ...
    public void onClick(View v)
    {
        switch (v.getId())
        {
            case R.id.writeExcel:
                saveExcelFile("quizExcel.xls");
                break;
            case R.id.iv_back:
                finish();
                break;
            case R.id.readExcel:
                mediaPath= Environment.getExternalStorageDirectory() + IMAGE_DIRECTORY+"/"+"quizExcel.xls";
                uploadFile();
                break;
        }
    }
    ...
    private void uploadFile() {
        ...
        if (mediaPath != null) {
            ...
            RequestBody type1 = RequestBody.create(MediaType.parse("type"), "document");
            ...
            RequestBody subject_id = RequestBody.create(MediaType.parse("subject_id"), "1");
            RequestBody title1 = RequestBody.create(MediaType.parse("title"), "kkjfglkfl");

            call = getResponse.uploadFile(school_id, teacher_id, type1, status, section_id, subject_id, title1, fileToUpload);
